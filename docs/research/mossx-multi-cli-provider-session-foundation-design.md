...
> Product visibility 校准：2026-08-25 · `doge-unified-product-subscription`：authenticated product UI 强制保留 Codex/Claude/Kimi，Rust settings read/update/restore 与 renderer normalization 都移除三者的 legacy visibility blacklist；shipping Settings/model menu 不再挂载 Engine Management，`providers/vendors/permissions` deep link 回退 Basic Settings。managed provider/configuration authority 未改变，每次 startup 仍由 `account_product_v1_prepare` 幂等 apply/verify 三引擎 Doge projection。事实源：`src-tauri/src/{types.rs,shared/settings_core.rs}`、`src/features/account/runtime/productManagedEnginePolicy.ts`、`src/features/settings/{hooks/useAppSettings.ts,components/SettingsView.tsx}`、`src/features/composer/components/ChatInputBox/selectors/ModelSelect.tsx`、OpenSpec `doge-unified-product-subscription` §16.42。
...
| Account-managed Provider binding | Codex/Claude Code/Kimi 在 mandatory Product Gate 中统一使用 bundled managed toolchain 与 stable `doge-token-matrix` binding；每次 authenticated startup 幂等 prepare。remote key 按 authenticated account scope 内的 `Composite group + hashed device` 识别；create response 不可信时由 Native 在同一次 prepare 内 exact authoritative re-list + Desktop handoff，renderer Retry 不拥有 reconcile。secret 不进 renderer/business config，Doge child 使用 isolated home/private settings。Shipping Engine Management 不挂载，legacy visibility blacklist 不能隐藏三种 product engine。Home、Shared Next Turn、Existing Native modification 与 Account Center 共享 refreshable product snapshot；existing binding immutable，跨 engine/provider 继续走 managed prepare + new session/Continuation。 | `src-tauri/src/account/{configuration.rs,authority.rs,runtime/runtime_product.rs,runtime/runtime_product_keys.rs,runtime_ipc.rs}`、`src-tauri/src/shared/settings_core.rs`、`src/features/account/{components/ProductAccountAppGate.tsx,runtime/productManagedEnginePolicy.ts,runtime/productPrepareRetry.ts}`、`src/features/settings/{hooks/useAppSettings.ts,components/SettingsView.tsx}`、`src/features/composer/components/ChatInputBox/selectors/{ProductEngineModelSelect.tsx,ModelSelect.tsx}`、OpenSpec `doge-unified-product-subscription` |
| Managed engine availability and Home target authority | Generic PATH detection is not authoritative for bundled engines；Product Gate 对 Codex/Claude/Kimi 都以 successful managed toolchain inspection + bundled selection 为 readiness truth。Home engine switch 必须保留 explicit `selectedCreationTarget` 直到 managed activation 完成，避免首发回落到其他 engine。 | `src/features/engine/hooks/{engineControllerAvailability.ts,useEngineController.ts}`、`src/features/account/runtime/productEngineProvisioning.ts`、`src/features/composer/components/Composer.tsx`、OpenSpec `doge-unified-product-subscription` |
