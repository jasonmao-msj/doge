...
| Account-managed Provider binding | Codex/Claude Code/Kimi 在 mandatory product AccountGate ready 后使用 stable id `doge-token-matrix` binding，用户显示名固定为 `Doge`；每次 authenticated startup 均 idempotent prepare，same-id entry 的 `managedRevision` 缺失/过期即 fail closed 并覆盖旧 Doge projection，其他 local/custom providers 保留。remote key 在 authenticated account scope 内按 `Composite group + hashed device` 识别，不依赖 mutable plan copy；same device/group 重试复用，cross-device identity 分离。secret 不进 renderer/business config，desktop launch 从 selected durable vault 注入；Codex/Kimi 仅给 Doge child 设置 isolated home，Claude 仅给 turn 设置 private `--settings`，terminal global config 不受影响。Product-ready Engine Management 锁定 local activation。Home、Shared Next Turn、Existing Native modification UI 与 Account Center 从一个 refreshable product snapshot 读取 upstream `id/display_name/model/compatible_engines/capabilities`；不再维护 exact model-id manifest。30s freshness + focus/visibility + 60s fallback + manual refresh 由 single in-flight owner 发布 last-known-good。product picker 只展示 Codex/Claude/Kimi，provider/configuration 不进入 UI；model commit 后 panel 保持打开。product-ready Sidebar 的三种 direct action 绕过 Local Mode blacklist。`selectedAtomicTarget` 同时拥有 trigger、readiness、persistence 与 send truth；stale/removed incompatible target 必须 repair 后才可发送。Existing Native Session binding 仍 immutable，跨 engine/provider 继续通过 managed prepare 创建 new session/Continuation。Checkout refund terminal fail closed；pending polling 保留 attempt/cooldown 并受 absolute expiry 约束。Catalog/endpoint smoke 不构成 compatibility truth，exact CLI Agent payload typed terminal仍是E2E证据；server policy/capacity failure不得 silent fallback。 | `src-tauri/src/account/{configuration.rs,authority.rs,runtime/runtime_product.rs,runtime_ipc.rs}`、`src-tauri/src/engine/{commands.rs,kimi_provider_profile.rs}`、`src/features/account/{components/ProductAccountAppGate.tsx,runtime/productEntitlementStore.ts,runtime/productModelCatalogRefresh.ts,runtime/productExecutionTarget.ts,runtime/productModelCompatibility.ts}`、`src/features/app/hooks/useSidebarMenus.ts`、`src/features/composer/components/ChatInputBox/selectors/ProductEngineModelSelect.tsx`、`src/features/vendors/components/{VendorSettingsPanel.tsx,VendorOfficialConfigCard.tsx}`、OpenSpec `doge-unified-product-subscription` |
| Account-managed Provider binding | Codex/Claude Code/Kimi 在 mandatory product AccountGate ready 后使用 stable id `doge-token-matrix` binding，用户显示名固定为 `Doge`；每次 authenticated startup 均 idempotent prepare，same-id entry 的 `managedRevision` 缺失/过期即 fail closed 并覆盖旧 Doge projection，其他 local/custom providers 保留。remote key 在 authenticated account scope 内按 `Composite group + hashed device` 识别，不依赖 mutable plan copy；same device/group 重试复用，cross-device identity 分离。secret 不进 renderer/business config，desktop launch 从 selected durable vault 注入；Codex/Kimi 仅给 Doge child 设置 isolated home，Claude 仅给 turn 设置 private `--settings`，terminal global config 不受影响。Product-ready Engine Management 锁定 local activation。Home、Shared Next Turn、Existing Native modification UI 与 Account Center 从一个 refreshable product snapshot 读取 upstream `id/display_name/model/compatible_engines/capabilities`；不再维护 exact model-id manifest。30s freshness + focus/visibility + 60s fallback + manual refresh 由 single in-flight owner 发布 last-known-good。product picker 只展示 Codex/Claude/Kimi，provider/configuration 不进入 UI；model commit 后 panel 保持打开。product-ready Sidebar 的三种 direct action 绕过 Local Mode blacklist。`selectedAtomicTarget` 同时拥有 trigger、readiness、persistence 与 send truth；stale/removed incompatible target 必须 repair 后才可发送。Existing Native Session binding 仍 immutable，跨 engine/provider 继续通过 managed prepare 创建 new session/Continuation。Checkout refund terminal fail closed；pending polling 保留 attempt/cooldown 并受 absolute expiry 约束。Catalog/endpoint smoke 不构成 compatibility truth，exact CLI Agent payload typed terminal仍是E2E证据；server policy/capacity failure不得 silent fallback。 | `src-tauri/src/account/{configuration.rs,authority.rs,runtime/runtime_product.rs,runtime_ipc.rs}`、`src-tauri/src/engine/{commands.rs,kimi_provider_profile.rs}`、`src/features/account/{components/ProductAccountAppGate.tsx,runtime/productEntitlementStore.ts,runtime/productModelCatalogRefresh.ts,runtime/productExecutionTarget.ts,runtime/productModelCompatibility.ts}`、`src/features/app/hooks/useSidebarMenus.ts`、`src/features/composer/components/ChatInputBox/selectors/ProductEngineModelSelect.tsx`、`src/features/vendors/components/{VendorSettingsPanel.tsx,VendorOfficialConfigCard.tsx}`、OpenSpec `doge-unified-product-subscription` |
| Managed engine availability and Home target authority | Generic PATH detection is not authoritative for bundled Codex; only a successful managed toolchain inspection with `status=ready` overlays Codex `installed/version/error`, while other engines retain generic detection and managed inspection failures remain visible in debug. A Home engine switch must preserve the explicit `selectedCreationTarget` until managed activation completes so the first send cannot fall back to Claude. | `src/features/engine/hooks/{engineControllerAvailability.ts,useEngineController.ts}`, `src/features/composer/components/Composer.tsx`, OpenSpec `doge-unified-product-subscription` |
