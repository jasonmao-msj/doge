...
> 初始日期：2026-07-27
> 最近校准：2026-08-17 · `require-account-engine-subscription-onboarding`：account-managed Codex/Claude 使用安装包内 pinned official engine；Claude binary 通过 provider-scoped config 注入，manual/local provider 继续使用用户配置，daemon send/compact 对该 binding fail closed
...
| Account-managed Provider binding | Codex/Claude Code 在 mandatory AccountGate ready 后使用 engine-scoped `doge-token-matrix` binding；secret 不进 renderer/config，desktop launch 从 OS vault 注入；bundled/external binary choice 只写 process-local account runtime，Claude 使用 `ClaudeSessionManager.provider_configs` 隔离 manual/local default；daemon send/compact 均不回退 local/default | `src-tauri/src/account/{runtime.rs,runtime_ipc.rs,toolchain.rs,configuration.rs}`、`src-tauri/src/engine/claude/{manager.rs,provider_profile.rs}`、`src-tauri/src/bin/doge_daemon/{daemon_state.rs,runtime_helpers.rs}`、OpenSpec `require-account-engine-subscription-onboarding` |
