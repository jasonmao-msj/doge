...
> 初始日期：2026-07-27
> 最近校准：2026-09-01 · `add-wechat-bridge-channel`：设置页只管理微信渠道生命周期，不持久化 global routing target；联系人通过微信指令按 `wxid` 独立持久化 `workspaceId + engine + providerProfileId + modelCatalogEntryId + model` 与 pending selection，同一 wxid 只在 exact target 相等时续接 canonical native session。Claude/Codex/OpenCode/Kimi/Grok sync send 使用 explicit provider-scoped runtime，Codex user-visible thread 禁止 helper hide/archive；turn durable success 后发布 `wechat://session-updated`，Renderer 只刷新匹配的当前 workspace catalog。事实源：`src-tauri/src/{wechat/mod.rs,wechat/target_commands.rs,engine/commands.rs,engine/codex_prompt_service.rs,types.rs}`、`src/{features/settings/components/settings-view/sections/WechatChannelSettings.tsx,features/threads/hooks/useThreads.ts,services/events.ts,types/wechat.ts}`、OpenSpec `add-wechat-bridge-channel`。
>
> 最近校准：2026-08-31 · `fix-managed-codex-first-turn-target-drift`：Product-ready Existing Native send 使用完整 `MessageExecutionTargetSnapshot` 贯穿 Atomic picker/readiness/dispatch；target unresolved、Engine mismatch 或已知 Provider mismatch 在 optimistic UI/Runtime side effect 前 fail closed。managed configuration revision=3，将 Codex isolated `model/review_model` 从被 Doge channel pricing 拒绝的`gpt-5.5`迁到`gpt-5.6-sol`；revision-2 existing install在first exact-engine prepare自动升级，Kimi/local/custom不变。Shared managed Codex 对 app-server 未转发的 provider-private image item，在 terminal commit 以 frozen Provider/native session/runtime Turn exact reconcile rollout，bounded content-addressed 落盘并只写 canonical compact artifact；committed UI refresh projection，preview仅allowlist exact App Data generated-image subtree。existing user升级后无需清配置/登录/会话，下一个Turn自动生效。事实源：`src-tauri/src/{shared_generated_image_artifact.rs,shared_session_v2.rs,shared_projection/projector.rs,workspaces/commands.rs,account/configuration.rs}`、`src/features/{composer/components/Composer.tsx,threads/hooks/useThreadMessaging.ts}`、OpenSpec `fix-managed-codex-first-turn-target-drift`。
...
> 最近校准：2026-08-27 · `lazy-engine-provisioning`：Product CLI readiness 从 startup Gate 移到 frozen send target boundary。active entitlement catalog 成立即 mount AppShell；`account_product_v1_prepare(engineId=null)` 只后台 reconcile Product credential/model catalog，失败不得 unmount/锁定 App。New Home、Existing Native、Shared 与 Sidebar direct create 在任何 Session/Binding/Turn side effect 前复用 `ensureProductEngineReadyV1`，只对 exact `doge-token-matrix + engine` 执行 config、toolchain inspect/install、post-install verification 与 activation；same-engine dedupe，Kimi one-shot，失败恢复 draft/attachments。bundled/external 已可用时整个 readiness transaction silent，只有实际 `cli_install_run` 才进入 bottom-right installing→ready card，禁止把 internal checking/configuring/activating 投影给用户。事实源：`src/features/account/{components/ProductAccountAppGate.tsx,runtime/productEngineProvisioning.ts}`、`src/features/{threads/hooks/useThreadMessaging.ts,app/hooks/useWorkspaceActions.ts,layout/hooks/useLayoutNodes.tsx}`、`src-tauri/src/account/{runtime/runtime_product.rs,toolchain.rs}`、OpenSpec `lazy-engine-provisioning`。
> 最近校准：2026-08-27 · `render-product-models-by-protocol`：Product model entitlement 先归一为 endpoint-level managed Provider API protocol，再投影到本地 Engine；该 API protocol 不等于 CLI process/stdout `protocolFamily`。Codex 消费 `openai-responses`、Kimi 消费 `openai-chat-completions`、Claude 消费 `anthropic-messages`，只有多 endpoint row 才可跨 engine；`OpenAI-compatible` family 或 `/v1/models` presence 不能替代 exact endpoint evidence。K3 初始 Responses 400 的根因是 production Composite route 缺口；补齐 `kimi*` / `k3*` → Kimi / Responses routes 后，`k3`、`k3-256k`、`kimi-for-coding` Responses 全部 200，故 Kimi-family 无 metadata fallback 归入 Responses + Chat Completions，在 Codex/Kimi 同时可用。explicit upstream protocol metadata 是 authority，legacy `compatible_engines` 只作为 endpoint evidence，unknown fail closed。事实源：`src-tauri/src/account/runtime/runtime_product_models.rs`、`src/features/account/runtime/{productOnboardingClient.ts,productModelCompatibility.ts}`、OpenSpec `render-product-models-by-protocol`。
> 最近校准：2026-08-27 · `unify-product-target-catalog-consumers`：Renderer 不再把 raw Product models 交给每个 selector 重算 compatibility，而由 `projectProductTargetCatalogV1` 一次投影为 `engines[].models[]` exact target rows，保留 `modelCatalogEntryId` 与 runtime `model`。Composer、Kanban 与 target repair 复用该 projection；Product Kanban 不得 fallback local `EngineStatus`，也不得把 entitlement 伪装成 installed。Kanban task new-write complete `ExecutionTarget`、flat fields dual-read；TaskRun/session/send 前 fresh resolve，recurring/retry/fork 保留 target，同 engine thread reuse 还必须匹配 `providerProfileId`，从而禁止 managed task 漂入 local Provider。事实源：`src/features/account/runtime/{productTargetCatalog.ts,productExecutionTarget.ts}`、`src/features/kanban/{components/TaskCreateModal.tsx,utils/taskTargetCatalog.ts,utils/kanbanStorage.ts}`、`src/app-shell-parts/{kanbanExecutionTarget.ts,useAppShellKanbanExecutionSection.ts}`、OpenSpec `unify-product-target-catalog-consumers`。
> 最近校准：2026-08-25 · `doge-unified-product-subscription`：product-ready Home 的 Codex availability authority 是已验证的 managed toolchain result；generic PATH probe 无法看到 bundled binary 时，只有 managed `status=ready` 才能覆盖 Codex 的 installed/version/error，Claude、Grok、Kimi、OpenCode 继续使用各自 generic probe，managed inspect 失败保持未覆盖并写入 debug。Home 对 Codex 的显式 target 选择必须在异步 managed activation 完成前保存在 `selectedCreationTarget`，首条消息只能使用该冻结 target，禁止回落到旧的 Claude default。事实源：`src/features/engine/hooks/{engineControllerAvailability.ts,useEngineController.ts}`、`src/features/composer/components/Composer.tsx`、OpenSpec `doge-unified-product-subscription`。
...
> Provider binding 校准：2026-08-23 · `doge-unified-product-subscription`：product-ready Home 的 `ComposerCreateSessionTarget` 是新 Native Session 首发的冻结 authority；Home/Kanban 即使先创建 pending thread，也必须把完整 target 保留到 immediate send boundary，不能依赖尚未 rerender 的 thread store。`useThreadMessaging` 必须对所有 engine 一并消费 target 的 engine/provider/model/effort，禁止只为 Codex 透传 Provider 或让 Claude 以 null binding 回落 ambient `~/.claude/settings.json`。既有 Native Session binding 仍 immutable，跨 Provider 继续走 Provider Continuation。事实源：`src/app-shell-parts/useAppShellKanbanComposerSection.ts`、`src/features/composer/components/Composer.tsx`、`src/features/threads/hooks/useThreadMessaging.ts`。
> Product authority 校准：2026-08-23 · `doge-unified-product-subscription`：token2api `Doge APP` 自定义 `/v1/models` 展示列表是产品 model release authority，Doge 继续动态消费且不维护 exact-id allowlist。product-ready Sidebar 的 Claude/Codex/Kimi 新建入口不再暴露 Provider submenu，统一直接创建 `doge-token-matrix` binding；非产品/Local Mode 保留 legacy Provider 管理。事实源：`src/features/app/hooks/useSidebarMenus.ts`、change verification §16.26–16.27。
> Managed configuration 校准：2026-08-24 · `doge-unified-product-subscription`：authenticated product startup 对 Codex/Claude/Kimi 每次执行 idempotent prepare；registry 的 same-id managed entry 必须携带 current `managedRevision`，缺失/过期即 fail closed 并完整覆盖旧 Doge projection，同时保留其他 local/custom providers。Doge-spawned Codex/Kimi child 使用 isolated home，Claude turn 使用 private `--settings`，不得污染 terminal global CLI config。product-ready Engine Management 锁定 local activation，但允许编辑 terminal-owned global file。事实源：`src-tauri/src/account/configuration.rs`、`src/features/vendors/components/{VendorSettingsPanel.tsx,VendorOfficialConfigCard.tsx}`、OpenSpec `doge-unified-product-subscription` §16.31。
> Product visibility 校准：2026-08-25（2026-08-27 provisioning owner 已更新）· `doge-unified-product-subscription`：authenticated product UI 强制保留 Codex/Claude/Kimi，Rust settings read/update/restore 与 renderer normalization 都移除三者的 legacy visibility blacklist；shipping Settings/model menu 不再挂载 Engine Management，`providers/vendors/permissions` deep link 回退 Basic Settings。visibility 不再代表 startup configuration readiness；exact engine Doge projection 由 `lazy-engine-provisioning` 的 send-time owner 收敛。事实源：`src-tauri/src/{types.rs,shared/settings_core.rs}`、`src/features/account/runtime/{productManagedEnginePolicy.ts,productEngineProvisioning.ts}`、`src/features/settings/{hooks/useAppSettings.ts,components/SettingsView.tsx}`、OpenSpec `doge-unified-product-subscription` §16.42 与 `lazy-engine-provisioning`。
> Product Gate/default 校准：2026-08-24 · `doge-unified-product-subscription`：shipping renderer 删除 legacy `AccountAppGate` 与 engine-scoped subscription/usage surface，main route 只有 `ProductAccountAppGate`。Product Home 的无显式选择初始 Target 固定为 Codex + current upstream order 中第一条 Codex-compatible model；global/local last-used selection 不参与该初始化，当前 Home 的明确选择仍保持到 Session create。事实源：`src/router.tsx`、`src/features/composer/components/Composer.tsx`、OpenSpec §16.33–16.34。
> Product recovery 校准：2026-08-24 · `doge-unified-product-subscription`：managed Composite remote key identity 固定为 authenticated account scope 下的 `group_id + hashed device_id`，禁止用 mutable plan copy 识别 credential；checkout refund 为 terminal failure，pending polling 由 stable `checkoutId/status/expiresAt` owner 持有 attempt、cooldown 与 absolute expiry。product-ready Sidebar 的 Codex/Claude/Kimi direct actions 不受 Local Mode blacklist 隐藏，model commit 后 product picker 保持打开。事实源：`src-tauri/src/account/runtime/runtime_product.rs`、`src/features/account/components/ProductAccountAppGate.tsx`、`src/features/app/hooks/useSidebarMenus.ts`、`src/features/composer/components/ChatInputBox/selectors/ProductEngineModelSelect.tsx`、OpenSpec §16.37–16.41。
> Product prepare reconcile 校准：2026-08-26 · `doge-unified-product-subscription`：managed-key create 属于 outcome-unknown mutation；`protocolMismatch`、transport uncertainty 或 invalid success 不能证明 side effect 未发生。Native 必须在同一次 prepare 内按 exact Composite group + deterministic hashed-device name authoritative re-list，命中 active key 后经 owner-authorized Desktop handoff继续；禁止把第二次 UI Retry 当作 reconcile owner。事实源：`src-tauri/src/account/runtime/{runtime_product.rs,runtime_product_keys.rs}`、`.trellis/spec/backend/account-convenience-native-contract.md`、OpenSpec §16.43。
> Provider Continuation hydration 校准：2026-08-24 · `fix-provider-continuation-target-hydration`：Native Continuation `ready` 后 frontend 必须先 await exact destination engine/model/effort hydration，再选择 target thread；不得依赖同一 React batch 内 stale catalog closure，也不得用 source active-session setter 补写 target。事实源：`src/features/app/hooks/useSidebarMenus.ts`、`src/app-shell-parts/providerContinuationTargetHydration.ts`、OpenSpec `fix-provider-continuation-target-hydration`。
> Native provisional identity 校准：2026-08-24 · `fix-product-provisional-thread-terminal-settlement`：Product Home logical thread 可能是无 engine prefix 的 UUID；只要 `engineSource + activeTurnId` 与 session hint exact match，就必须 promotion 到 canonical Native identity。Kimi/Grok/Claude/Gemini/OpenCode canonical terminal 必须在 same-tick rename 场景 settle exact-turn alias，禁止依赖 rerender 或从 assistant text 猜 terminal。事实源：`src/features/threads/utils/threadPendingResolution.ts`、`src/features/threads/hooks/useThreadTurnEvents.ts`、OpenSpec `fix-product-provisional-thread-terminal-settlement`。
...
> 最近校准：2026-08-30 · `enable-cross-protocol-product-engine-models`：production `Doge APP` 为`claude-*`建立Responses→Anthropic raw-prefix route；minimal Responses与真实managed Codex CLI `claude-opus-4-8` turn均completed/`OK`。metadata-absent Claude-family canonical protocol facts因此收敛为`openai-responses + anthropic-messages`，由`projectProductTargetCatalogV1`一次投影到Codex+Claude并排除Kimi；explicit upstream metadata仍可收窄。managed routing contract bump `ACCOUNT_MANAGED_CONFIGURATION_REVISION=2`，fresh user直接写current projection，revision-1 old user在first exact-engine send side effect前deterministic migrate，login/catalog prepare保持non-blocking。事实源：`src-tauri/src/account/{runtime/runtime_product_models.rs,configuration.rs}`、`src/features/account/runtime/{productModelCompatibility.ts,productTargetCatalog.ts}`、OpenSpec `enable-cross-protocol-product-engine-models`。
> 最近校准：2026-08-28 · `sync-upstream-stability-2026-08`：基于 `upstream/main@cd362f8cf` 对 476 个分叉后提交做 capability-driven selective sync。Terminal path 采用 upstream `0d8f2426c` 的 causal drain + exact-turn late full-snapshot salvage：terminal barrier 前同步清 legacy/normalized/contract batcher 与 live tail，barrier 后仅完整 assistant completion 可 content-only 补账，禁止复燃 processing/第二 terminal。Codex provider facts 采用 unknown context window 不伪造 200K、managed scope 不拼 official ghost fallback、runtime reasoning metadata fill-only、GUI/daemon `env_key` Rust-only bounded resolution；Product catalog 仍以 endpoint-level API protocol → `projectProductTargetCatalogV1` 为唯一 authority。拒绝 upstream 90s 零首事件 timer 直接清 processing，因为正文/无事件 timeout 不是 terminal authority。事实源：`src/features/threads/{contracts/realtimeEventContract.ts,hooks/useThreadItemEvents.ts,hooks/useThreadEventHandlers.ts}`、`src-tauri/src/{engine/codex_adapter.rs,engine/status.rs,codex/provider_env.rs,backend/app_server.rs}`、OpenSpec `sync-upstream-stability-2026-08`。
...
| Native/Shared terminal text causal integrity | externalized live text、normalized contract batcher 与 late complete snapshot 以 exact Turn 为 scope：terminal 前同步 drain；terminal 后仅 non-empty assistant complete 可 content-only salvage，生命周期仍只信 typed/scoped terminal | `src/features/threads/contracts/realtimeEventContract.ts`、`src/features/threads/hooks/{useThreadItemEvents.ts,useThreadEventHandlers.ts}`、OpenSpec `sync-upstream-stability-2026-08` |
| Codex provider-scoped model/env/usage facts | managed catalog 不拼 provider 未声明的 official fallback；discovery reasoning metadata 保留且仅按 runtime identity 补缺；context window 缺失保持 unknown；`env_key` 仅由 Rust launch owner bounded resolve，Doge native launch_env 优先且 secret 不进 renderer/log | `src-tauri/src/{engine/status.rs,engine/codex_adapter.rs,codex/provider_env.rs,backend/app_server.rs}`、`src/features/{models/atomicModelReasoning.ts,composer/components/ChatInputBox/hooks/useProviderTargetCatalogOwners.ts}`、OpenSpec `sync-upstream-stability-2026-08` |
| Managed Codex hosted image exposure | `doge-token-matrix` cold launch从exact binary导出bundled catalog，只把GPT-5.6 trio设为non-Lite并以launch-scoped catalog生效；failure pre-side-effect fail closed，local/custom provider不变；图片完成只认native image item/payload | `src-tauri/src/codex/{managed_model_catalog.rs,session_runtime.rs,provider_profile.rs}`、OpenSpec `enable-managed-codex-image-generation` |
| Product-managed Native first-turn target | Existing Native UI/readiness/send共享完整target snapshot，unresolved/mismatch零发送；managed revision 3以Sol作为Codex config-only fallback，explicit Terra/Luna仍按frozen target发送 | `src-tauri/src/account/configuration.rs`、`src/features/composer/components/Composer.tsx`、`src/features/threads/hooks/useThreadMessaging.ts`、OpenSpec `fix-managed-codex-first-turn-target-drift` |
| Shared managed Codex image artifact | exact provider/native-session/runtime-Turn rollout reconcile；20 MiB bounded image原子落App Data，canonical只存compact ref；terminal refresh current projection，preview只开放exact managed image root | `src-tauri/src/{shared_generated_image_artifact.rs,shared_session_v2.rs,shared_projection/projector.rs,workspaces/commands.rs}`、`src/features/threads/hooks/useThreadMessaging.ts`、OpenSpec `fix-managed-codex-first-turn-target-drift` |
...
| WeChat channel Native target | 设置页不保存 global target；联系人通过微信指令按 wxid 独立保存 exact workspace/engine/provider/model 与 pending selection。同一 wxid 仅在 exact target 相等时续接 canonical user-visible native session，sync send 不得回落 global provider；durable success 后定向刷新 workspace catalog | `src-tauri/src/{wechat/mod.rs,wechat/target_commands.rs,engine/commands.rs,engine/codex_prompt_service.rs}`、`src/{features/settings/components/settings-view/sections/WechatChannelSettings.tsx,features/threads/hooks/useThreads.ts}`、OpenSpec `add-wechat-bridge-channel` |
| Session target cold-start authority | Native 可变 model/reasoning selection 使用 durable execution target，renderer cache 仅作兼容 fallback；Shared read authority 为 V2 selected target > legacy meta，read path 不写回，hydration pending 禁止 Product default repair | `src-tauri/src/{session_management.rs,shared_sessions.rs}`、`src/{app-shell-parts/useSelectedComposerSession.ts,features/composer/components/Composer.tsx}`、OpenSpec `fix-managed-session-target-cold-start` |
| Product model API protocol projection | Product upstream row归一为`openai-responses|openai-chat-completions|anthropic-messages`；explicit metadata是authority。metadata-absent Kimi/K3经双OpenAI endpoint evidence进入Codex+Kimi；Claude-family经verified Responses→Anthropic route与真实Codex terminal进入Codex+Claude并排除Kimi。Renderer不保留第二份per-engine catalog authority | `src-tauri/src/account/runtime/runtime_product_models.rs`、`src/features/account/runtime/{productModelCompatibility.ts,productTargetCatalog.ts}`、OpenSpec `render-product-models-by-protocol`与`enable-cross-protocol-product-engine-models` |
| Product target catalog consumer parity | `projectProductTargetCatalogV1` 是 Product Renderer 的唯一 per-engine target projection；Composer/Kanban/repair 复用 `engines[].models[]`，Kanban 持久化 exact `ExecutionTarget` 并在 TaskRun/session/send 前校验 managed Provider + runtime model；thread reuse key 至少为 `engine + providerProfileId` | `src/features/account/runtime/productTargetCatalog.ts`、`src/features/kanban/utils/taskTargetCatalog.ts`、`src/app-shell-parts/kanbanExecutionTarget.ts`、OpenSpec `unify-product-target-catalog-consumers` |
| Provider Continuation ready hydration | `ready` 后 destination-first：显式收敛 target engine，写 exact target per-thread model/effort，完成后才导航；same binding model click 不再创建第二个 Continuation | `src/features/app/hooks/useSidebarMenus.ts`、`src/app-shell-parts/providerContinuationTargetHydration.ts`、OpenSpec `fix-provider-continuation-target-hydration` |
| Native provisional identity promotion | conventional pending prefix 与 Product Home unprefixed UUID 都只能通过 exact engine/turn owner promotion；canonical terminal same-tick settle matching alias，active selection/text 不构成 authority | `src/features/threads/{utils/threadPendingResolution.ts,hooks/useThreadTurnEvents.ts}`、OpenSpec `fix-product-provisional-thread-terminal-settlement` |
| Account-managed Provider binding | Codex/Claude Code/Kimi 使用 stable `doge-token-matrix` binding，但 Product Gate 只以 active entitlement catalog 决定 AppShell mount。startup catalog-only prepare 不 apply engine config；send-time exact-engine prepare 才写 selected Doge projection。remote key 按 authenticated account scope 内的 `Composite group + hashed device` 识别；create response 不可信时由 Native 在同一次 prepare 内 exact authoritative re-list + Desktop handoff，renderer Retry 不拥有 reconcile。secret 不进 renderer/business config，Doge child 使用 isolated home/private settings。Shipping Engine Management 不挂载，legacy visibility blacklist 不能隐藏三种 product engine。 | `src-tauri/src/account/{configuration.rs,authority.rs,runtime/runtime_product.rs,runtime/runtime_product_keys.rs,runtime_ipc.rs}`、`src/features/account/{components/ProductAccountAppGate.tsx,runtime/productEngineProvisioning.ts}`、OpenSpec `lazy-engine-provisioning` |
| Kimi native history root / catalog recovery | `custom_home=None` 时由 `kimi_history` 统一解析 default + managed provider homes；`session_index.jsonl.workDir` 是 workspace ownership authority，`providerProfileId` 仅为 metadata overlay；enumeration failure 为 partial/degraded，且 first-paint 无 live signal 时由 Kimi one-shot durable seed 补齐 catalog。 | `src-tauri/src/engine/kimi_history.rs`、`src-tauri/src/session_management_catalog_projection.rs`、`src/features/threads/hooks/useThreadActions.threadList.ts`、OpenSpec `2026-08-30-fix-kimi-managed-session-catalog` |
| Managed engine availability and Home target authority | Generic PATH detection is not authoritative for shipping bundled engines；exact send target 的 readiness truth 是 coordinator 完成 Product access + managed toolchain inspection/optional exact installer + post-install verification + required activation。same-engine calls dedupe，Kimi 不依赖 global activation。Home 必须保留 explicit `selectedCreationTarget` 到该 transaction 完成，失败发生在 Session/Binding/Turn side effect 前。 | `src/features/account/runtime/productEngineProvisioning.ts`、`src/features/{threads/hooks/useThreadMessaging.ts,app/hooks/useWorkspaceActions.ts,composer/components/Composer.tsx}`、`src-tauri/src/account/toolchain.rs`、OpenSpec `lazy-engine-provisioning` |
