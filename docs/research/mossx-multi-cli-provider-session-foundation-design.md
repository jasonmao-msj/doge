...
| Product model API protocol projection | Product upstream row 归一为 `openai-responses|openai-chat-completions|anthropic-messages`；Codex/Kimi 不因同属 OpenAI family 共享全部 rows，Renderer 不保留第二份 per-engine catalog authority | `src-tauri/src/account/runtime/runtime_product_models.rs`、`src/features/account/runtime/productModelCompatibility.ts`、OpenSpec `render-product-models-by-protocol` |
