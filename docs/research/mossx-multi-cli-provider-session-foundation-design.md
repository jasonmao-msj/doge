...
> 初始日期：2026-07-27
> 最近校准：2026-08-23 · `doge-unified-product-subscription`：product-ready Home 与 Shared Next Turn 使用同一动态 product entitlement catalog；上游 `compatible_engines` 存在时直接收窄，缺失时只对已知 GPT/Claude/Kimi/豆包 family 使用 adapter fallback，unknown family fail closed。incompatible engine switch 原子 fallback，空交集 fail closed，provider 固定 `doge-token-matrix`。Product picker 隐藏 provider/configuration，readiness 与 send 只读 exact `ExecutionTarget`。Existing Native Session 继续保持 immutable engine/provider binding。Model entitlement、minimal endpoint probe 与真实 CLI Agent payload compatibility 分层取证；typed terminal 作为真实 E2E 证据，server policy/capacity failure 禁止 silent fallback 或伪造 client identity 绕过。
...
| Account-managed Provider binding | Codex/Claude Code/Kimi 在 mandatory product AccountGate ready 后使用 `doge-token-matrix` binding；secret 不进 renderer/business config，desktop launch 从 selected durable vault 注入。Product-ready Home、Shared Next Turn 与 Account Center 从一个 refreshable product snapshot 读取 upstream `id/display_name/model/compatible_engines/capabilities`；不再维护 exact model-id manifest。30s freshness + focus/visibility + 60s fallback + manual refresh 由 single in-flight owner 发布 last-known-good。provider/configuration 不进入 product picker。`selectedAtomicTarget` 同时拥有 trigger、readiness、persistence 与 send truth；stale/removed incompatible target 必须 repair 后才可发送。Existing Native Session 保持 immutable engine/provider binding，跨 engine/provider 继续创建 new session/Continuation。Catalog/endpoint smoke 不构成 compatibility truth，exact CLI Agent payload typed terminal仍是E2E证据；server policy/capacity failure不得 silent fallback。 | `src-tauri/src/account/{authority.rs,runtime/runtime_product.rs,runtime_ipc.rs}`、`src-tauri/src/engine/{commands.rs,kimi_provider_profile.rs}`、`src/features/account/runtime/{productEntitlementStore.ts,productModelCatalogRefresh.ts,productExecutionTarget.ts,productModelCompatibility.ts}`、`src/features/composer/components/{Composer.tsx,ChatInputBox/selectors/ProductEngineModelSelect.tsx}`、OpenSpec `doge-unified-product-subscription` |
