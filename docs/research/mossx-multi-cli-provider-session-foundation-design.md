...
| Account-managed Provider binding | Codex/Claude Code/Kimi 在 mandatory product AccountGate ready 后使用 stable id `doge-token-matrix` binding，用户显示名固定为 `Doge`；每次 authenticated startup 均 idempotent prepare，same-id entry 的 `managedRevision` 缺失/过期即 fail closed 并覆盖旧 Doge projection，其他 local/custom providers 保留。secret 不进 renderer/business config，desktop launch 从 selected durable vault 注入；Codex/Kimi 仅给 Doge child 设置 isolated home，Claude 仅给 turn 设置 private `--settings`，terminal global config 不受影响。Product-ready Engine Management 锁定 local activation。Home、Shared Next Turn、Existing Native modification UI 与 Account Center 从一个 refreshable product snapshot 读取 upstream `id/display_name/model/compatible_engines/capabilities`；不再维护 exact model-id manifest。30s freshness + focus/visibility + 60s fallback + manual refresh 由 single in-flight owner 发布 last-known-good。product picker 只展示 Codex/Claude/Kimi，provider/configuration 不进入 UI。`selectedAtomicTarget` 同时拥有 trigger、readiness、persistence 与 send truth；stale/removed incompatible target 必须 repair 后才可发送。Existing Native Session binding 仍 immutable，跨 engine/provider 继续通过 managed prepare 创建 new session/Continuation。Catalog/endpoint smoke 不构成 compatibility truth，exact CLI Agent payload typed terminal仍是E2E证据；server policy/capacity failure不得 silent fallback。 | `src-tauri/src/account/{configuration.rs,authority.rs,runtime/runtime_product.rs,runtime_ipc.rs}`、`src-tauri/src/engine/{commands.rs,kimi_provider_profile.rs}`、`src/features/account/runtime/{productEntitlementStore.ts,productModelCatalogRefresh.ts,productExecutionTarget.ts,productModelCompatibility.ts}`、`src/features/vendors/components/{VendorSettingsPanel.tsx,VendorOfficialConfigCard.tsx}`、OpenSpec `doge-unified-product-subscription` |
