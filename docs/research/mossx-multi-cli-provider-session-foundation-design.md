...
> 初始日期：2026-07-27
> 最近校准：2026-08-27 · `render-product-models-by-protocol`：Product model entitlement 先归一为 managed Provider API protocol，再投影到本地 Engine；该 API protocol 不等于 CLI process/stdout `protocolFamily`。Codex 与 Kimi 共享 `openai` catalog，Claude 消费 `anthropic` catalog，双协议 row 可投影到三引擎；explicit upstream protocol metadata 是 authority，legacy `compatible_engines` 只作为 protocol evidence，unknown fail closed。事实源：`src-tauri/src/account/runtime/runtime_product_models.rs`、`src/features/account/runtime/{productOnboardingClient.ts,productModelCompatibility.ts}`、OpenSpec `render-product-models-by-protocol`。
