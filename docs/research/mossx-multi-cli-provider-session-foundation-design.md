...
> 初始日期：2026-07-27
> 最近校准：2026-08-19 · `refine-account-shortcut-and-managed-provider-default`：AccountGate 只有在 engine prepare 与 `doge-token-matrix` provider activation 均成功后，才把 Codex/Claude 新会话默认绑定到托管 provider；Home 按该 scope 刷新 model catalog，catalog 未就绪时禁发，绝不回退旧 local Key；existing session、Local Mode 与显式 local/manual target 保持不变
...
| Account-managed Provider binding | Codex/Claude Code 在 mandatory AccountGate ready 后使用 engine-scoped `doge-token-matrix` binding；secret 不进 renderer/config，desktop launch 从 OS vault 注入；bundled/external binary choice 只写 process-local account runtime，Claude 使用 `ClaudeSessionManager.provider_configs` 隔离 manual/local default；daemon send/compact 均不回退 local/default。`prepared` 只在 account prepare、engine activation 与 provider activation 都成功后成立；Home/new-session 仅在该状态下加载同 scope catalog 并作为默认 target，catalog 未就绪时禁发而不是冒充 local；existing session、Local Mode 和显式 local/manual target 不受覆盖 | `src-tauri/src/account/{runtime.rs,runtime_ipc.rs,toolchain.rs,configuration.rs}`、`src-tauri/src/engine/claude/{manager.rs,provider_profile.rs}`、`src-tauri/src/bin/doge_daemon/{daemon_state.rs,runtime_helpers.rs}`、`src/features/account/{components/AccountAppGate.tsx,runtime/engineEntitlementStore.ts}`、`src/app-shell-parts/useProviderModelCatalogSync.ts`、`src/features/composer/{components/Composer.tsx,components/ChatInputBox/ChatInputBox.tsx,utils/resolveDefaultCreationExecutionTarget.ts}`、OpenSpec `require-account-engine-subscription-onboarding`、`refine-account-shortcut-and-managed-provider-default` |
