...
> 初始日期：2026-07-27
> 最近校准：2026-08-20 · `refine-account-shortcut-and-managed-provider-default`：AccountGate 只有在 engine prepare 与 `doge-token-matrix` provider activation 均成功后，才把 Codex/Claude 新会话默认绑定到托管 provider；Home 按该 scope 刷新 model catalog，catalog 未就绪时禁发，绝不回退旧 local Key；单次 create-session 的显式 local/manual target 在离开创建态后清理，既有 session 继续读取 durable binding，下一次新建重新解析托管默认值；既有 local/manual Native session 同 engine 显式切到 Token Matrix 时，先重新执行幂等 prepare 校验 OS-vault credential，再由 ready signal 打开 managed 新会话，禁止先创建注定失败的 Provider Continuation
...
| Account-managed Provider binding | Codex/Claude Code 在 mandatory AccountGate ready 后使用 engine-scoped `doge-token-matrix` binding；secret 不进 renderer/config，desktop launch 从 OS vault 注入；bundled/external binary choice 只写 process-local account runtime，Claude 使用 `ClaudeSessionManager.provider_configs` 隔离 manual/local default；daemon send/compact 均不回退 local/default。`prepared` 只在 account prepare、engine activation 与 provider activation 都成功后成立；Home/new-session 仅在该状态下加载同 scope catalog 并作为默认 target，catalog 未就绪时禁发而不是冒充 local。Home 的显式 local/manual target 只属于当次 creation attempt，离开 create-session mode 后清理；created/existing session 继续读取 durable binding，下一次新建重新解析 managed default。既有 Native local/manual binding 显式切到同 engine 的 Token Matrix 时仍须进入 AccountGate prepare；该交互会重验 server-owned binding 与 OS-vault credential，不能拿可能陈旧的 renderer `prepared` snapshot 直接放行 Provider Continuation | `src-tauri/src/account/{runtime.rs,runtime_ipc.rs,toolchain.rs,configuration.rs}`、`src-tauri/src/engine/claude/{manager.rs,provider_profile.rs}`、`src-tauri/src/bin/doge_daemon/{daemon_state.rs,runtime_helpers.rs}`、`src/features/account/{components/AccountAppGate.tsx,runtime/engineEntitlementStore.ts}`、`src/app-shell-parts/useProviderModelCatalogSync.ts`、`src/features/composer/{components/Composer.tsx,components/ChatInputBox/ChatInputBox.tsx,utils/resolveDefaultCreationExecutionTarget.ts}`、OpenSpec `require-account-engine-subscription-onboarding`、`refine-account-shortcut-and-managed-provider-default` |
