...
fn build_detected_run_script(launcher: &RuntimeLaunchCommand) -> String {
    ...
    let mut lines = vec![
        "DOGE_RUN_EXIT_CODE=0".to_string(),
        "if ! command -v java >/dev/null 2>&1; then".to_string(),
        "  echo \"[doge Run] Java not found. Install JDK and ensure java is on PATH.\"".to_string(),
        "  DOGE_RUN_EXIT_CODE=127".to_string(),
        "else".to_string(),
    ];
    ...
    match launcher.kind {
        RuntimeLauncherKind::MavenWrapper => {
            ...
            lines.push("    echo \"[doge Run] ./mvnw is missing execute permission. Run: chmod +x ./mvnw\"".to_string());
            lines.push("    DOGE_RUN_EXIT_CODE=126".to_string());
            lines.push("  else".to_string());
            lines.push(format!("    echo \"[doge Run] Using: {joined}\""));
            lines.push(format!("    {joined}"));
            lines.push("    DOGE_RUN_EXIT_CODE=$?".to_string());
            lines.push("  fi".to_string());
        }
        RuntimeLauncherKind::MavenSystem => {
            lines.push("  if ! command -v mvn >/dev/null 2>&1; then".to_string());
            lines.push("    echo \"[doge Run] Maven not found. Install Maven or add ./mvnw to project root.\"".to_string());
            lines.push("    DOGE_RUN_EXIT_CODE=127".to_string());
            lines.push("  else".to_string());
            lines.push(format!("    echo \"[doge Run] Using: {joined}\""));
            lines.push(format!("    {joined}"));
            lines.push("    DOGE_RUN_EXIT_CODE=$?".to_string());
        ...
        RuntimeLauncherKind::GradleWrapper => {
            ...
            lines.push("    echo \"[doge Run] ./gradlew is missing execute permission. Run: chmod +x ./gradlew\"".to_string());
            lines.push("    DOGE_RUN_EXIT_CODE=126".to_string());
            lines.push("  else".to_string());
            lines.push(format!("    echo \"[doge Run] Using: {joined}\""));
            lines.push(format!("    {joined}"));
        ...
        RuntimeLauncherKind::GradleSystem => {
            ...
            lines.push("    echo \"[doge Run] Gradle not found. Install Gradle or add ./gradlew to project root.\"".to_string());
            lines.push("    DOGE_RUN_EXIT_CODE=127".to_string());
            lines.push("  else".to_string());
            lines.push(format!("    echo \"[doge Run] Using: {joined}\""));
            lines.push(format!("    {joined}"));
    ...
    lines.push("fi".to_string());
    lines.push("echo \"[doge Run] __EXIT__:${DOGE_RUN_EXIT_CODE}\"".to_string());
...
fn build_detected_run_script(launcher: &RuntimeLaunchCommand) -> String {
    ...
    let mut lines = vec![
        ...
        "set \"DOGE_RUN_EXIT_CODE=0\"".to_string(),
        "where java >nul 2>&1".to_string(),
        "if errorlevel 1 (".to_string(),
        "  echo [doge Run] Java not found. Install JDK and ensure java is on PATH.".to_string(),
        "  set \"DOGE_RUN_EXIT_CODE=127\"".to_string(),
        ") else (".to_string(),
    ];
    ...
    match launcher.kind {
        RuntimeLauncherKind::MavenWrapper => {
            ...
            lines.push(format!(
                "    echo [doge Run] {} not found in project root.",
                launcher.program
            ));
            lines.push("    set \"DOGE_RUN_EXIT_CODE=126\"".to_string());
            lines.push("  ) else (".to_string());
            lines.push(format!("    echo [doge Run] Using: {joined}"));
            lines.push(format!("    call {joined}"));
            lines.push("    set \"DOGE_RUN_EXIT_CODE=!ERRORLEVEL!\"".to_string());
            lines.push("  )".to_string());
        }
        RuntimeLauncherKind::MavenSystem => {
            lines.push("  where mvn >nul 2>&1".to_string());
            lines.push("  if errorlevel 1 (".to_string());
            lines.push("    echo [doge Run] Maven not found. Install Maven or add mvnw.cmd to project root.".to_string());
            lines.push("    set \"DOGE_RUN_EXIT_CODE=127\"".to_string());
            lines.push("  ) else (".to_string());
            lines.push(format!("    echo [doge Run] Using: {joined}"));
            lines.push(format!("    call {joined}"));
            lines.push("    set \"DOGE_RUN_EXIT_CODE=!ERRORLEVEL!\"".to_string());
        ...
        RuntimeLauncherKind::GradleSystem => {
            ...
            lines.push("  if errorlevel 1 (".to_string());
            lines.push("    echo [doge Run] Gradle not found. Install Gradle or add gradlew.bat to project root.".to_string());
            lines.push("    set \"DOGE_RUN_EXIT_CODE=127\"".to_string());
            lines.push("  ) else (".to_string());
            lines.push(format!("    echo [doge Run] Using: {joined}"));
            lines.push(format!("    call {joined}"));
...
fn build_shell_run_script(
    ...
    let mut lines = vec!["DOGE_RUN_EXIT_CODE=0".to_string()];
    if let Some(program) = dependency_program {
        ...
        lines.push("else".to_string());
        lines.push(format!(
            "  echo \"[doge Run] Using {source_label}: {command}\""
        ));
...
fn build_shell_run_script(
    ...
    if let Some(program) = dependency_program {
        ...
        lines.push("  set \"DOGE_RUN_EXIT_CODE=127\"".to_string());
        lines.push(") else (".to_string());
        lines.push(format!("  echo [doge Run] Using {source_label}: {command}"));
        lines.push(format!("  {command}"));
        lines.push("  set \"DOGE_RUN_EXIT_CODE=!ERRORLEVEL!\"".to_string());
        lines.push(")".to_string());
    } else {
...
fn build_python_profile_script(command: &str) -> String {
    ...
    [
        ...
        format!("  echo \"[doge Run] Using detected runtime profile: python3 {executable_args}\""),
        format!("  python3 {executable_args}"),
        "  DOGE_RUN_EXIT_CODE=$?".to_string(),
        "elif command -v python >/dev/null 2>&1; then".to_string(),
        format!("  echo \"[doge Run] Using detected runtime profile: python {executable_args}\""),
        format!("  python {executable_args}"),
        "  DOGE_RUN_EXIT_CODE=$?".to_string(),
        "else".to_string(),
        "  echo \"[doge Run] Python not found. Install Python and ensure `python` or `python3` is available on PATH.\"".to_string(),
        "  DOGE_RUN_EXIT_CODE=127".to_string(),
        "fi".to_string(),
        "echo \"[doge Run] __EXIT__:${DOGE_RUN_EXIT_CODE}\"".to_string(),
    ]
    .join("\n")
}
...
fn build_python_profile_script(command: &str) -> String {
    ...
    [
        ...
        format!("  echo [doge Run] Using detected runtime profile: py -3 {executable_args}"),
        format!("  call py -3 {executable_args}"),
        "  set \"DOGE_RUN_EXIT_CODE=!ERRORLEVEL!\"".to_string(),
        ") else (".to_string(),
        "  where python >nul 2>&1".to_string(),
        "  if errorlevel 1 (".to_string(),
        "    echo [doge Run] Python not found. Install Python and ensure `py` or `python` is available on PATH.".to_string(),
        "    set \"DOGE_RUN_EXIT_CODE=127\"".to_string(),
        "  ) else (".to_string(),
        format!("    echo [doge Run] Using detected runtime profile: python {executable_args}"),
