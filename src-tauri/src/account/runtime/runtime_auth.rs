...
impl AccountRuntime {
    pub(super) async fn execute_operation(
        ...
        match operation {
            ...
            // Password reset is exposed only after the Desktop handoff/ticket
            // continuation is implemented end-to-end. Sending a legacy email
            // alone would advertise an App flow that cannot safely complete.
            "auth.requestPasswordReset" => Err(capability_failure("recover")),
    ...
    pub(super) fn read_human_requirement(&self, state: &RuntimeState) -> Result<Value, Value> {
        match state.public_settings.as_ref() {
            Some(settings) if settings.turnstile_enabled => Ok(json!({
                "status": "unavailable",
    ...
    pub(super) async fn begin_registration(
        ...
        if !settings.registration_enabled || settings.turnstile_enabled {
