...
pub(crate) struct PublicSettingsWire {
    ...
    #[serde(default)]
    pub(crate) tencent_captcha_enabled: bool,
    #[serde(default)]
    pub(crate) aliyun_captcha_enabled: bool,
    pub(crate) api_base_url: Option<String>,
    pub(crate) version: Option<String>,
}

impl PublicSettingsWire {
    pub(crate) fn requires_human_verification(&self) -> bool {
        self.turnstile_enabled || self.tencent_captcha_enabled || self.aliyun_captcha_enabled
    }
...
pub(crate) struct DesktopEngineCatalogWire {
    pub(crate) engines: Vec<DesktopEngineWire>,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
pub(crate) struct DesktopEngineWire {
    pub(crate) id: String,
    pub(crate) display_name: String,
    pub(crate) entitlement: DesktopEngineEntitlementWire,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
pub(crate) struct DesktopEngineEntitlementWire {
    pub(crate) status: String,
    pub(crate) subscription_id: Option<i64>,
    pub(crate) group_id: Option<i64>,
    pub(crate) expires_at: Option<String>,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
pub(crate) struct DesktopEnginePlansWire {
    pub(crate) engine_id: String,
    pub(crate) plans: Vec<DesktopSubscriptionPlanWire>,
    pub(crate) payment_methods: Vec<DesktopPaymentMethodWire>,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
pub(crate) struct DesktopSubscriptionPlanWire {
    pub(crate) id: i64,
    pub(crate) name: String,
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub(crate) struct DesktopPaymentMethodWire {
    pub(crate) id: String,
    pub(crate) display_name: String,
    pub(crate) currency: String,
}
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub(crate) struct DesktopCheckoutActionWire {
...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub(crate) struct DesktopCheckoutWire {
...
#[derive(Clone, Debug, Deserialize)]
pub(crate) struct DesktopManagedAccessWire {
    pub(crate) engine_id: String,
...
impl TokenMatrixAuthority {
    ...
    pub(crate) async fn desktop_engines(
        &self,
        access_token: &str,
    ) -> Result<DesktopEngineCatalogWire, AuthorityError> {
        self.request(
            Method::GET,
            "/api/v1/desktop/v1/engines",
            None,
            Some(access_token),
            None,
        )
    ...
    pub(crate) async fn desktop_engine_plans(
        &self,
        access_token: &str,
        engine_id: &str,
    ) -> Result<DesktopEnginePlansWire, AuthorityError> {
        self.request(
            Method::GET,
            &format!("/api/v1/desktop/v1/engines/{engine_id}/plans"),
            None,
            Some(access_token),
            None,
        )
    ...
    pub(crate) async fn create_desktop_checkout(
        &self,
        access_token: &str,
        engine_id: &str,
        plan_id: i64,
        payment_type: &str,
        operation_id: &str,
    ) -> Result<DesktopCheckoutWire, AuthorityError> {
        self.request(
            Method::POST,
            &format!("/api/v1/desktop/v1/engines/{engine_id}/checkouts"),
            Some(json!({
                "plan_id": plan_id,
    ...
    pub(crate) async fn desktop_checkout(
        &self,
        access_token: &str,
        checkout_id: i64,
    ) -> Result<DesktopCheckoutWire, AuthorityError> {
        self.request(
            Method::GET,
            &format!("/api/v1/desktop/v1/checkouts/{checkout_id}"),
            None,
            Some(access_token),
            None,
        )
    ...
    pub(crate) async fn ensure_desktop_managed_access(
        &self,
        access_token: &str,
        engine_id: &str,
        device_id: &str,
        operation_id: &str,
    ) -> Result<DesktopManagedAccessWire, AuthorityError> {
        self.request(
            Method::POST,
            &format!("/api/v1/desktop/v1/engines/{engine_id}/managed-access"),
...
fn map_reason(status: StatusCode, reason: &str) -> &'static str {
    match reason {
        ...
        "DESKTOP_ENGINE_SUBSCRIPTION_REQUIRED" => "subscriptionRequired",
        "PLAN_NOT_AVAILABLE" => "planUnavailable",
        "DESKTOP_ENGINE_NOT_FOUND" | "DESKTOP_ENGINE_UNSUPPORTED" => "capabilityUnavailable",
        "DESKTOP_DEVICE_ID_INVALID" | "DESKTOP_CHECKOUT_INVALID" => "validationRejected",
...
mod tests {
    ...
    async fn production_adapter_parses_current_envelope_and_sends_scoped_headers() {
        ...
        let app = Router::new()
            ...
            .route(
                ...
                post(|Json(body): Json<Value>| async move {
                    assert_eq!(body, json!({ "email": "synthetic@example.invalid" }));
                    Json(json!({
                        "code": 0,
                        "data": {
                            "message": "neutral anti-enumeration response"
                        }
                    }))
                }),
            )
        ...
        authority
            .request_password_reset("synthetic@example.invalid")
            .await
            .expect("password reset request accepted");
    }
