...
- Full batched CI MAY为 transient runner scheduling对 failed test做一次 Vitest retry；MUST保留默认 5000ms test timeout，MUST NOT让 local/default callers继承 retry，且 retry MUST bounded `<=3`。
...
| local/default batched Vitest | `retry=0` | 隐式放宽所有 callers |
...
- Good：batched CI单次 5s jitter由 `--retry 1` 恢复；同一测试连续失败两次仍阻断 CI。
...
- Bad：把所有 test timeout从 5s提高到 15s，或对本地/default开启 retry。
...
- `node --test scripts/test-batched.test.mjs` MUST assert default/CI retry config、invalid/out-of-range rejection 与 exact Vitest args；CI static contract MUST assert only `test-js` / `test-windows` set `VITEST_RETRY: "1"`。
