...
- Focused frontend suites 26/26 pass；frontend boundary + toolchain 6/6 pass；Rust account 82 pass、2 isolated-live tests ignored；build-script tests 3/3 pass；`npm run typecheck`、`npm run lint`（0 errors，16 pre-existing warnings）、`npm run check:runtime-contracts`、`cargo fmt -- --check` 与 OpenSpec strict validation 均 pass。
