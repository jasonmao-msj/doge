...
follow-up [CI run 32818081850](https://github.com/jasonmao-msj/doge/actions/runs/32818081850)：同一 Rust full suite 重跑通过，确认前次 process-group failure 为 timing flake；Windows 越过 provenance test，推进到 batch 88 后暴露 CSS CRLF exact-source assertion，现已由 shared `normalizeSourceText()` 修复并增加显式 CRLF regression。`test-js` 此次被未触碰的 `FileViewPanel.capture-note` shortcut selection timing flake 阻断；上一 run 全量 JS 已通过。最终 follow-up CI 待记录。

final follow-up [CI run 32819427440](https://github.com/jasonmao-msj/doge/actions/runs/32819427440)：`typecheck`、`lint`、`docs`、`memory-kind-contract`、`test-js` 全量与 `build-macos` 通过；Rust 唯一失败仍为同一未触碰 process-group timing test。Windows 越过前两项修复后推进到更后面的 `settings-basic-light-controls`，再次证明 exact CSS source 需要 line-ending normalization；现已抽取 `src/test/sourceText.ts#normalizeSourceText`，同时接入 Composer selector 与 Settings basic tests，focused 3 files / 9 tests + ESLint pass。最终 Windows rerun 待记录。
