...
- [x] 16.23 Shared assistant turn label 隐藏 managed provider display name，仅展示 engine、model 与可选 reasoning effort；保留 provider identity 作为内部 binding/dedupe fact。
- [x] 16.24 修复 Home `createSessionTarget` 在 `useThreadMessaging` 被丢弃、且首次 Provider 仅对 Codex 透传的缺口；Claude/Kimi 等新 Native Session 统一消费冻结 target 的 engine/provider/model/effort，避免回落 ambient local profile。
- [x] 16.25 从 `Claude 官方定价` 精确复制 `claude-opus-5 + claude-opus-4-8` 官方 `5/25/6.25/0.5 $/MTok` 规则到 `Doge 统一定价`；managed Messages HTTP 与隔离 private-settings Claude CLI 均验证 Opus 4.8 typed success。
- [x] 16.26 实测上游 13 个 Claude ids：Messages 仅 Opus 4.8 / Opus 5 / Sonnet 4.6 通过，exact Claude CLI 仅 Opus 4.8 稳定成功（Opus 5=403 daily limit，Sonnet 4.6=bounded timeout）；在 token2api UI 将 Claude #11/#41/#42 账号白名单与 `Doge APP` 自定义 `/v1/models` 展示列表都收敛到 `claude-opus-4-8`，Doge 不写死 model id。
- [x] 16.27 product-ready Sidebar 新建菜单移除 Claude/Codex/Kimi Provider submenu；三项直接使用 canonical `Doge` managed profile 创建会话，Local Mode legacy menu 保持兼容。
- [x] 16.28 修复 Home/Kanban 首发竞态：wrapper 预创建 pending Claude thread 后不得删除 `createSessionTarget`；在 reducer 尚未发布 thread binding 时，send boundary 继续以冻结 target 的 `doge-token-matrix` 为 Provider authority。
- [x] 16.29 复用 token2api 现有 usage stats/model snapshot：Account Center 移除副标题、quota/engine filler 与 engine block；model requests/tokens/actual/standard cost 合并为语义化表格；`current|previous` 改为 preset/custom date range + `day|hour` validated Native query；range Popover 固定 opaque surface；provider/LLM icon 统一由 shared `original | mono-adaptive | dark-tile` theme strategy 管理。
- [x] 16.30 将 snapshot trend 投影到 Account Center：Input/Output/Cache Creation/Cache Read + Cache Hit Rate 双轴图，legend 可逐线隐藏/恢复；model table 固定高度滚动；core Token formatter 统一 uppercase `K/M/B`。
- [x] 16.31 加固 product managed configuration authority：增加 `managedRevision` 使旧 Doge same-id registry 在每次 authenticated prepare 时 fail-closed 并确定性覆盖；Codex/Claude/Kimi 继续使用 child-only isolated home/private settings，不改 terminal global config；Engine Management 的 local activation 在 product-ready 时置灰锁定并保留仅供 terminal 使用的 edit 入口。
- [x] 16.32 修复旧安装包抢占 hot 验收：确认截图进程为 `/Applications/doge.app` 且 WebView 为 `tauri://localhost`，源码 router 仍唯一挂载 `ProductAccountAppGate`；新增 macOS packaged-process preflight，使 hot/isolated/signed dev 在 `.app` 占用 canonical single-instance identity 时 fail closed，避免旧 `AccountAppGate` 冒充当前源码。
- [x] 16.33 清退 renderer 旧按引擎订阅产物：删除 `AccountAppGate`、engine subscription/usage panels、engine checkout client/preferences/activation wrapper 及专属 CSS/copy；Sidebar account shortcut 直接消费统一 product snapshot；共享 view/style 重命名为 Product owner。
...
- [x] 16.44 将 Product model compatibility 归一为 endpoint-level managed Provider API protocol：Codex=Responses、Kimi=Chat Completions、Claude=Anthropic Messages；explicit protocol metadata authoritative，unknown fail closed；实现与回归由 OpenSpec `render-product-models-by-protocol` 跟踪。
- [x] 16.45 定位 K3 Responses 400 为 production Composite route 缺口；经用户授权补齐 `kimi*` / `k3*` → Kimi / Responses routes，managed-key 复测 `k3`、`k3-256k`、`kimi-for-coding` 全部 200，Kimi-family fallback 据此投影到 Codex + Kimi。
- [x] 16.46 **Superseded startup behavior by OpenSpec `lazy-engine-provisioning`**：active entitlement 立即 mount AppShell；catalog-only Product prepare 后台执行，CLI/config/toolchain 只在 frozen send target 第一次使用时按 exact engine 收敛，进度与失败不得锁定 App。
