...
Active OpenSpec changes in the current working tree (31; see [`changes/README.md`](changes/README.md)):
...
Archived `fix-wechat-windows-outbound-file-links` after L3 focused verification. Synced the `wechat-bridge-channel` requirement covering Windows slash-prefixed drive normalization, `.md` outbound `file_item` classification, native macOS/Linux absolute-path preservation, and host-native CI fixtures. Real-device transfer smoke and L4 cross-platform package validation remain release/CI residuals.

Current counts: active=`31`, archive=`892`, specs=`518`.
