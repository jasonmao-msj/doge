...
type AccountExperienceControllerV1 = ReturnType<typeof useAccountExperienceControllerV1>;
...
export function AccountExperience() {
  ...
  return (
    <div className="account-experience" data-testid="account-experience">
      ...
      {signedIn
        ? <AccountCenter controller={controller} />
        : <AccountAuth controller={controller} />}
...
function AccountLocalModeStatus() {
  const copy = useAccountExperienceCopyV1();
  return (
    <div className="account-local-mode-status">
      <span><Check size={14} aria-hidden />{copy.localModeBadge}</span>
      <AccountHelpTooltip label={`${copy.help}：${copy.localModeBadge}`}>
        {copy.localModeNote}
      </AccountHelpTooltip>
    </div>
  );
...
function AccountAuth({ controller }: { readonly controller: AccountExperienceControllerV1 }) {
  const copy = useAccountExperienceCopyV1();
...
function AccountCenter({ controller }: { readonly controller: AccountExperienceControllerV1 }) {
  const copy = useAccountExperienceCopyV1();
  ...
  return (
    <section className="account-center" aria-labelledby="account-center-title">
      ...
      <Tabs value={controller.centerTab} onValueChange={(value) => {
        ...
        <TabsPanel value="security"><SecurityPanel controller={controller} /></TabsPanel>
      </Tabs>
    </section>
  );
}
...
function SecurityPanel({ controller }: { readonly controller: AccountExperienceControllerV1 }) {
  const copy = useAccountExperienceCopyV1();
  ...
  return (
    <div className="account-security-panel">
      ...
      <dl className="account-security-list">
        ...
        <div><dt><KeyRound aria-hidden />{copy.codexCredential}</dt><dd>{controller.configuration.managedKeyReady ? copy.enabled : copy.disabled}</dd></div>
      </dl>
