...
const EXIT_CODE_PATTERN = /\[(?:doge|ccgui|CodeMoss) Run\] __EXIT__:(-?\d+)/;
...
function buildLegacyJavaRunScript(commandOverride?: string | null) {
  if (IS_WINDOWS_RUNTIME) {
    ...
    if (normalizedOverride) {
      return [
        ...
        "set \"DOGE_RUN_EXIT_CODE=0\"",
        "where java >nul 2>&1",
        "if errorlevel 1 (",
        "  echo [doge Run] Java not found. Install JDK and ensure java is on PATH.",
        "  set \"DOGE_RUN_EXIT_CODE=127\"",
        ") else (",
        "  echo [doge Run] Using custom run command from console.",
        `  ${normalizedOverride}`,
        "  set \"DOGE_RUN_EXIT_CODE=!ERRORLEVEL!\"",
        ")",
        "echo [doge Run] __EXIT__:!DOGE_RUN_EXIT_CODE!",
      ].join("\r\n");
    }
    ...
    return [
      ...
      "setlocal EnableExtensions EnableDelayedExpansion",
      "set \"DOGE_RUN_EXIT_CODE=0\"",
      "echo [doge Run] Detecting Java launcher...",
      "where java >nul 2>&1",
      "if errorlevel 1 (",
      "  echo [doge Run] Java not found. Install JDK and ensure java is on PATH.",
      "  set \"DOGE_RUN_EXIT_CODE=127\"",
      ") else if exist \"mvnw.cmd\" if exist \"pom.xml\" (",
      "  echo [doge Run] Using: mvnw.cmd spring-boot:run",
      "  call mvnw.cmd spring-boot:run",
      "  set \"DOGE_RUN_EXIT_CODE=!ERRORLEVEL!\"",
      ") else if exist \"pom.xml\" (",
      "  where mvn >nul 2>&1",
      "  if errorlevel 1 (",
      "    echo [doge Run] Maven not found. Install Maven or add mvnw.cmd to project root.",
      "    set \"DOGE_RUN_EXIT_CODE=127\"",
      "  ) else (",
      "    echo [doge Run] Using: mvn spring-boot:run",
      "    call mvn spring-boot:run",
      "    set \"DOGE_RUN_EXIT_CODE=!ERRORLEVEL!\"",
      "  )",
      ") else if exist \"gradlew.bat\" if exist \"build.gradle\" (",
      "  echo [doge Run] Using: gradlew.bat bootRun",
      ...
      "  if errorlevel 1 (",
      "    echo [doge Run] Gradle not found. Install Gradle or add gradlew.bat to project root.",
      "    set \"DOGE_RUN_EXIT_CODE=127\"",
      "  ) else (",
      "    echo [doge Run] Using: gradle bootRun",
      "    call gradle bootRun",
      "    set \"DOGE_RUN_EXIT_CODE=!ERRORLEVEL!\"",
      "  )",
      ...
      "  )",
      ") else (",
      "  echo [doge Run] No Java project launcher detected in workspace root.",
      "  echo [doge Run] Expected one of: pom.xml, build.gradle, build.gradle.kts.",
      "  set \"DOGE_RUN_EXIT_CODE=127\"",
      ")",
      "echo [doge Run] __EXIT__:!DOGE_RUN_EXIT_CODE!",
    ].join("\r\n");
  ...
  if (normalizedOverride) {
    return [
      "DOGE_RUN_EXIT_CODE=0",
      "if ! command -v java >/dev/null 2>&1; then",
      "  echo \"[doge Run] Java not found. Install JDK and ensure java is on PATH.\"",
      "  DOGE_RUN_EXIT_CODE=127",
      "else",
      "  echo \"[doge Run] Using custom run command from console.\"",
      `  ${normalizedOverride}`,
      "  DOGE_RUN_EXIT_CODE=$?",
  ...
  return [
    ...
    "  echo \"[doge Run] Using: ./mvnw spring-boot:run\"",
    ...
    "    DOGE_RUN_EXIT_CODE=$?",
    "  else",
    "    echo \"[doge Run] Maven not found. Install Maven or add ./mvnw to project root.\"",
    "    DOGE_RUN_EXIT_CODE=127",
    "  fi",
    "elif [ -f \"./gradlew\" ] && { [ -f \"./build.gradle\" ] || [ -f \"./build.gradle.kts\" ]; }; then",
    "  echo \"[doge Run] Using: ./gradlew bootRun\"",
    "  ./gradlew bootRun",
    "  DOGE_RUN_EXIT_CODE=$?",
    ...
    "    DOGE_RUN_EXIT_CODE=$?",
    "  else",
    "    echo \"[doge Run] Gradle not found. Install Gradle or add ./gradlew to project root.\"",
    "    DOGE_RUN_EXIT_CODE=127",
    "  fi",
    "else",
    "  echo \"[doge Run] No Java project launcher detected in workspace root.\"",
...
export function useRuntimeLogSession({
  ...
  const onRunProject = useCallback(async () => {
    ...
    appendWorkspaceLog(
      workspaceId,
      `\n[doge Run] Starting at ${new Date().toLocaleTimeString()}\n`,
    );
    try {
      ...
      if (isMissingCommandError(error)) {
        try {
          ...
          appendWorkspaceLog(
            workspaceId,
            `[doge Run] Failed to start runtime: ${fallbackMessage}\n`,
          );
          return;
        }
      }
      ...
      appendWorkspaceLog(
        workspaceId,
        `[doge Run] Failed to start runtime: ${message}\n`,
      );
    }
  }, [
    activeDetectedProfiles,
  ...
  const onStopProject = useCallback(async () => {
    ...
    try {
      ...
      appendWorkspaceLog(workspaceId, "[doge Run] Stopped.\n");
    } catch (error) {
      if (isMissingCommandError(error)) {
        try {
          ...
          appendWorkspaceLog(
            workspaceId,
            `[doge Run] Stop failed: ${fallbackMessage}\n`,
          );
          return;
        }
      }
